...
jsBackend.pages.extras = {
    ...
    addBlockVisual: function(position, index, extraId, visible, extraType, extraData)
    {
        ...
        var showEditLink = (extraId != 0 && editLink);
        ...
        var blockHTML = '<div class="templatePositionCurrentType' + (visible ? ' ' : ' templateDisabled') + '" data-block-id="' + index + '">' +
            ...
            '<a href="' + (editLink ? editLink : '#') + '" class="btn btn-primary btn-icon-only btn-xs' + '"' + (showEditLink ? ' target="_blank"' : '') + (showEditLink ? '' : ' onclick="return false;"') + ((showEditLink) || extraId == 0 ? '' : 'style="display: none;" ') + '><span class="fa fa-pencil"></span></a>' +
    ...
    showAddDialog: function(e)
    {
        ...
        if ($('#addBlock').length > 0) {
            $('#addBlockSubmit').unbind('click').on('click', function(e) {
                ...
                // close dialog
                $('#addBlock').off('hidden.bs.modal').on('hidden.bs.modal', function() {}).modal('hide');
    ...
    {
        var html = '<div class="panel panel-default" id="user-template-textarea-' + key + '">';
...
jsBackend.pages.template = {
    ...
    changeTemplate: function()
    {
        ...
        $('#editContent .contentBlock').each(function(i)
        {
            ...
            if (current != old && $.inArray(extraId, old.data.default_extras[position]) >= 0) $('input[id=blockPosition' + index + ']', this).val('fallback');
        });
